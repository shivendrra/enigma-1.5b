...
# read the data
with open('new_dna.txt', 'r', encoding='utf-8') as f:
  text = f.read()
...
# hyperparameters
batch_size = 10 # independent sequences process in parallel
...
checkpoint_path = 'enigma_40k.pth'
checkpoint = torch.load(checkpoint_path)
model.load_state_dict(checkpoint)
m = model.to(device)

n_param = sum(p.numel() for p in m.parameters())/1e4
print(f"{n_param:.2f}, k parameters")
...
# saving the model
torch.save(model.state_dict(), f'enigma_{n_param:.0f}k.pth')
